...
def roughly_equal( a, b, tol=0.00075 ):
    return abs( a - b ) <= tol
...
class FloatSet:
    ...
    def __iter__( self ):
        ...
        for vv in self.values[1:]:
            if not roughly_equal( vv, temp[-1], 0.05 ):
                temp.append( vv )
        self.values = temp
...
class SimpleMs1Controller(Controller):
    # CJ.. this is the one we are actually using
    def __init__(self, mass_spec, upper_mz=None):
        super().__init__(mass_spec)
        default_scan = ScanParameters()
        default_scan.set(ScanParameters.MS_LEVEL, 1)
        if upper_mz:
            default_scan.set(ScanParameters.ISOLATION_WINDOWS, [[(0, upper_mz) ]])
        else:
    ...
    def peak_recorder( self ):
        return self.mass_spec.peak_recorder
    ...
    def create_eics( self, mapper ):
        # TODO should probably check for the ms level
        eics = []
        for chem, values in self.peak_recorder().items():
            # so we loop through all of the peaks and 
            # determine the unique mzs
            unique_mzs = FloatSet( [ pr[1] for pr in values ] )
            for currmz in unique_mzs:
                sliced = list(filter(lambda pr: roughly_equal( pr[1], currmz, 0.05), values ))
                curr_rts = np.array( mapper.rts() )
                curr_mz  = np.repeat( currmz, mapper.length() )
                curr_it  = np.zeros( mapper.length() )
                for (rt, mz, it ) in sliced:
                    idx = mapper.rt_to_idx( rt )
                    curr_it[ idx ] = it
                eics.append(
                        EIC( name=chem, rts=curr_rts, mzs=curr_mz, its=curr_it ) 
                        )
        return eics
    
    def _reset_scans( self, level ):
        num_scans = len( self.scans[level] )
        for idx in range( num_scans ):
            self.scans[ level ][ idx ].mzs = []
            self.scans[ level ][ idx ].intensities = [] 
            self.scans[ level ][ idx ].num_peaks = 0

    def update_scans( self, finalized_eics, mapper ):
        finalized_eics.sort( key=lambda eic: eic.mzs[0] )
        # first. reset each scan. we assume level is 1
        self._reset_scans( 1 )
        for eic in finalized_eics:
            assert len(eic.rts) == len(self.scans[1])
            for idx, (rt, mz, it) in enumerate(zip(eic.rts, eic.mzs, eic.its)):
                self.scans[ 1 ][ idx ].mzs.append( mz )
                self.scans[ 1 ][ idx ].intensities.append( it )
                self.scans[ 1 ][ idx ].num_peaks += 1

        num_scans = len( self.scans[1] )
        for idx in range( num_scans ):
            self.scans[ 1 ][ idx ].mzs = np.array( self.scans[ 1 ][ idx ].mzs )
            self.scans[ 1 ][ idx ].intensities = np.array( self.scans[ 1 ][ idx ].intensities )
