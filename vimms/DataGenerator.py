...
class PeakSampler(LoggerMixin):
    ...
    def get_msn_noisy_intensity(self, intensity, ms_level, max_intensity, pct):
        # CJ: for now, we will just be doing it where we randomly take a percentage of the 
        # max intensity... so we will need to have two extra arguments here
        ...
        assert 0 <= pct and pct <= 100, f"pct value must be on the range of [0, 100], {pct} is invalid."
        
        mult = (np.random.rand()*float(pct)) / float(100.)
        noise = max_intensity * mult
